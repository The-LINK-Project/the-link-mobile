...
import { buildDailyMix, DAILY_MIX_ID, dailyMix } from "../data/review";
...
describe("daily mix", () => {
    it("draws a short run from every lesson's exercises", () => {
        const mix = buildDailyMix(lessons, new Date(2026, 8, 15));
    ...
    it("is the same all day and different the next day", () => {
        const morning = buildDailyMix(lessons, new Date(2026, 8, 15, 7));
        const evening = buildDailyMix(lessons, new Date(2026, 8, 15, 22));
        const tomorrow = buildDailyMix(lessons, new Date(2026, 8, 16, 7));
